...
def load_data(input_dir) -> list:
    input_path = pathlib.Path(input_dir)

    data = input_path.read_text().split(',')
    return [int(ele) for ele in data]
...
def run_program(data):
    ...
    while True:
        if data[current_ind] == 99:
            ...
            input_1 = data[input_1_ind]

            input_2_ind = data[current_ind + 2]
            input_2 = data[input_2_ind]

            output_ind = data[current_ind + 3]

            log.debug("OpCode 1, adding %d and %d at indexes (%d, %d) and writing to index %d", input_1,
                      input_2, input_1_ind, input_2_ind, output_ind)
            data[output_ind] = input_1 + input_2

        elif data[current_ind] == 2:
            input_1_ind = data[current_ind + 1]
            input_1 = data[input_1_ind]
            ...
            log.debug("OpCode 2, multiplying %d and %d at indexes (%d, %d) and writing to index %d", input_1,
                      input_2, input_1_ind, input_2_ind, output_ind)
            data[output_ind] = input_1 * input_2

        else:
            raise ValueError(f"Unknown opcode {data[current_ind]} at position {current_ind}")

        # step forward
        current_ind += 4


def part_one(input_dir):
    data = load_data(input_dir)

    # bespoke logic for 1202
    data[1] = 12
    data[2] = 2
