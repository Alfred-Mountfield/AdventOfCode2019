...
def get_raw_fuel_per_mass(mass: int) -> int:
    return int(mass / 3) - 2


def get_fuel_for_modules(input_dir: str):
    ...
    with input_path.open('r') as input_file:
        ...
        while mass:
            total_fuel += get_raw_fuel_per_mass(int(mass))
            mass = input_file.readline()
...
def get_total_fuel_per_mass(mass: int) -> int:
    log.debug(f"Input mass is: {mass}")
    total = 0
    extra_fuel = get_raw_fuel_per_mass(int(mass))
    while extra_fuel > 0:
        log.debug(f"Which requires a fuel of mass: {extra_fuel}")
        total += extra_fuel
        extra_fuel = get_raw_fuel_per_mass(extra_fuel)

    return total


def get_total_fuel(input_dir: str):
    input_path = pathlib.Path(input_dir)
    ...
    with input_path.open('r') as input_file:
        ...
        while mass:
            total += get_total_fuel_per_mass(int(mass))
            mass = input_file.readline()
...
if __name__ == "__main__":
    ...
    log.info(f"Part 1: {get_fuel_for_modules('inputs/1.txt')}")
    log.info(f"Part 2: {get_total_fuel('inputs/1.txt')}")
